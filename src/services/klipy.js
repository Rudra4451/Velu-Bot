...
async function fetchFromKlipy(query, retries = 1) {
  ...
  try {
    ...
    logger.warn(`Klipy request failed for "${query}": ${err.message}`);
    if (retries > 0) {
...
export const klipyService = {
   ...
   *
   * @param {string} category - Fallback key (e.g. 'hug')
   * @param {string} query    - Klipy search query
