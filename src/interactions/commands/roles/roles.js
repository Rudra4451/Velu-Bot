...
export async function execute(interaction, client) {
  ...
  if (subcommand === 'create') {
    ...
    const invalidRoles = rawRoles.filter(role => role.position >= botMember.roles.highest.position);

    if (invalidRoles.length > 0) {
      const invalidMentions = invalidRoles.map(role => `${role}`).join(', ');
    ...
    const roleData = rawRoles.map(role => ({ id: role.id, name: role.name }));
    ...
    return middleware.safeReply(interaction, { embeds: [replyEmbed], ephemeral: true });
  }
