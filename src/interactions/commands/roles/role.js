...
function validateRoleHierarchy(interaction, role) {
...
export async function execute(interaction) {
  ...
  if (role) {
    ...
    if (!check.valid) {
      const embed = UIFactory.error('Hierarchy Error', check.reason);
      return middleware.safeReply(interaction, { embeds: [embed], ephemeral: true });
    }
  }
  ...
  if (subcommand === 'color') {
    ...
    await middleware.safeDefer(interaction);
    try {
      await role.setColor(hex, `Role color updated by ${interaction.user.tag}`);
