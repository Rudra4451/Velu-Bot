...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  await middleware.safeReply(interaction, { embeds: [embed], components: menuRow ? [menuRow as any] : [] });
}

export function buildC4Menu(game: any, refKey: string) {
...
export function renderC4Embed(game: any) {
  let grid = '';
...
export function dropChecker(board: any[], col: number, player: string) {
...
export function checkC4Win(board: any[]) {
  // Horizontal
  for (let r = 0; r < ROWS; r++) {
