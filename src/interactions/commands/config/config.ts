...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (subcommand === 'welcome') {
    ...
    if (autorole) db.updateConfig(guildId, 'welcomeAutoRole', autorole.id);
  ...
  if (subcommand === 'goodbye') {
    ...
    if (channel) db.updateConfig(guildId, 'goodbyeChannel', channel.id);
    if (message) db.updateConfig(guildId, 'goodbyeMessage', message);
    if (enabled !== null) db.updateConfig(guildId, 'goodbyeEnabled', enabled);
  ...
  if (subcommand === 'automod') {
    ...
    if (enabled !== null) db.updateConfig(guildId, 'automodEnabled', enabled);
    if (spamFilter !== null) db.updateConfig(guildId, 'automodSpamFilter', spamFilter);
    if (blockInvites !== null) db.updateConfig(guildId, 'automodBlockInvites', blockInvites);
    if (badwords !== null) db.updateConfig(guildId, 'automodBadwords', badwords);
    if (badwordsList) {
      const words = badwordsList.split(',').map(w => w.trim().toLowerCase()).filter(Boolean);
      db.updateConfig(guildId, 'automodBadwordsList', words);
    }
